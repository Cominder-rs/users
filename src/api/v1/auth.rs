...
use users_proto::*;
use beijing::*;
use civilization::utils::{make_internal, validate_regex};
...
use users_entities::{
    ...
    pending_registries::{self, Entity as PendingRegistry},
    ...
    async fn create_user(&self, request: Request<NewUser>) -> Result<Response<Token>, Status> {
        ...
        let pending_registry = PendingRegistry::find()
            ...
            .ok_or(Status::invalid_argument(
                AuthError::PendingRegistryNotFound.to_string(),
            ))?;
...
.last_insert_id;
    ...
    async fn get_country(&self, request: Request<()>) -> Result<Response<Country>, Status> {
        ...
        if let Some(ip_addr) = ip_addr {
            ...
            let country =
                utils::find_country(ip_addr, ip_db.to_owned()).unwrap_or(CountryCode::Unknown);
